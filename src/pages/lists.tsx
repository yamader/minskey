...
import { UserList, useListTimeline, useLists } from '~/features/list'
...
const inputClass = 'w-full rounded-md border-2 p-2 shadow-none focus:border-lime-400 focus:outline-none'
const actionClass = 'rounded-md bg-stone-700 px-3 py-1 font-bold text-white hover:bg-stone-600 active:bg-stone-500'
  ...
  return (
    <>
      <form
        ...
        />
        <button type="submit" className="rounded-md bg-lime-500 px-3 py-1 font-bold text-white hover:bg-lime-400">
          作成
        </button>
      </form>
...
function ListRow({ list, onChanged }: { list: UserList; onChanged: () => void }) {
  ...
  return (
    <div className="flex items-center gap-2 border-b bg-white p-3">
      {editing ? (
        <>
          <input className={inputClass} value={name} onChange={e => setName((e.target as HTMLInputElement).value)} />
        ...
        <>
          <Link className="grow" href={`${base}?list=${list.id}`}>
            ...
            {list.userIds && <div className="text-neutral-500 text-sm">{list.userIds.length} users</div>}
          </Link>
    ...
    <>
      ...
      <div className="border-b bg-white p-3">
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          {list?.userIds?.map(id => (
            <span key={id} className="flex items-center gap-1 rounded bg-neutral-100 px-2 py-0.5 text-xs">
              {id}
              <button type="button" className="text-neutral-500 hover:text-red-500" onClick={() => removeMember(id)}>
                ✕
              </button>
            </span>
          ))}
