import { Bell, ChevronsLeft, ChevronsRight, Home, LayoutGrid, List, Pen, Settings, User } from 'lucide-preact'
...
export default function DeckNav() {
  ...
  const open = (item: { sub: string | null }) => router.push(item.sub ? `/deck/${item.sub}` : '/deck')

  return (
    <div className={'flex h-full shrink-0 flex-col border-r bg-white transition-all ' + (collapsed ? 'w-16' : 'w-52')}>
      <div className="p-2">
        <button
          type="button"
          title="新しい投稿"
