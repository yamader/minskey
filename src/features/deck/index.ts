...
export function syncDeckWindows(url: string) {
  ...
  else if (list[idx].url !== url)
    openWindowsSignal.value = [...list.slice(0, idx), { ...list[idx], url }]
}
...
export function deckLabel(display: HomeDisplay, lists: UserList[]) {
  if (display.startsWith("list:"))
    return lists.find(list => `list:${list.id}` === display)?.name ?? "リスト"
...
export function useDeckStream(display: HomeDisplay) {
  ...
  useEffect(() => {
    ...
    setNotes([])
    const res = isList
      ? api.listTimeline(listId, { limit: 20 })
      : api.notes(display as TLNames, { limit: 20 })
    res.then(r => {
      if (r?.length) {
        setNotes(r)
  ...
  const more = useCallback(async () => {
    ...
    const res = isList
      ? await api.listTimeline(listId, opts)
      : await api.notes(display as TLNames, opts)
    if (res?.length) {
